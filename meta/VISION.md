# Vision: Pali Canon Vault

*Written 2026-05-22 by Claude Sonnet 4.6, informed by conversation with Roy Peter.*
*Updated 2026-05-27: Standardized inventory and defined boundary with companion NLP system.*

---

## Where We Are

The vault is a **practicing Buddhist's canonical library**: a structured environment for reading Pali suttas alongside their commentaries, with doctrinal cross-referencing via the mātikā web. It supports serious study and active practice without requiring external reference books.

Current inventory: 1,161 validated vault files (1,169 total md), 7 DN suttas, 11 MN suttas, 11 SN saṃyuttas (225 suttas), 14 AN suttas, Dhammapada, Udāna, Sutta Nipāta, Itivuttaka, Theragāthā, Therīgāthā, 22 mātikā lists — all with three-layer structure where available.

---

## Guiding Principles

These principles should constrain what gets added. Not every good idea belongs here.

1. **Practice first, study second.** Add content and tools that deepen meditation or clarify practice decisions. Resist the pull toward exhaustive coverage for its own sake.
2. **Comprehensive within scope.** As the vault matures, fill gaps systematically. A practicing Buddhist needs breadth across the canonical practice domains, not just depth in samatha.
3. **Quality over quantity.** Better to have fewer suttas with full three-layer structure and strong mātikā links than to race to complete coverage.
4. **Evaluate before the next phase.** After each tranche of new content, spend time actually reading it in Obsidian before commissioning more. The vault should drive the practice, not the reverse.

---

## Practice Domains (Priority Order)

The vault now serves seven interconnected practice domains. Content and tooling decisions should be guided by this priority:

### 1. Jhāna (Meditative Absorption)
*Deepening concentration practice through the four jhānas and formless attainments.*

Core texts already in vault: MN 111, AN 9.36, DN 2, DN 9, MN 52, AN 4.123–126, AN 5.28.
Texts to add:
- MN 128 (Upakkilesasutta — imperfections/obstacles in jhāna; critical for practitioners)
- AN 9.34–35 (Nibbāna and cessation through jhāna progression)
- MN 8 (Sallekhasutta — self-effacement as jhāna foundation)
- AN 8.63 (Saṅkhittasutta — brief instruction on jhāna with brahmavihāra)
- New mātikā: `four_jhanas` (cattāri jhānāni)

### 2. Vipassanā (Insight Meditation)
*Broadening insight practice beyond satipaṭṭhāna into the full framework of direct seeing.*

Core texts already in vault: MN 10, DN 22, SN 22, SN 35, MN 118, SN 47.
Texts to add:
- MN 148 (Chachakkasutta — six sets of six; comprehensive vipassanā framework)
- SN 36 (Vedanāsaṃyutta — feeling as vipassanā entry point; key for noting practice)
- MN 28 (Mahāhatthipadopamasutta — great elephant footprint; four elements contemplation)
- MN 140 (Dhātuvibhaṅgasutta — six elements, equanimity instruction)
- MN 2 (Sabbāsavasutta — removing taints; the 7-method framework)

### 3. Brahmavihāra (Sublime States)
*Mettā, karuṇā, muditā, upekkhā as both standalone practice and jhāna support.*

Core texts already in vault: Snp 1.8 (Karaṇīya Metta Sutta), `four_sublime_states` mātikā.
Texts to add:
- MN 7 (Vatthūpamasutta — simile of the cloth; brahmavihāra practice instruction)
- DN 13 (Tevijjasutta — brahmavihāra as path to Brahmā-companionship)
- AN 11.15 (Mettānisaṃsasutta — eleven benefits of mettā practice)
- AN 4.125–126 (Mettāsutta — mettā and rebirth destinations)
- SN 46.54 (Mettāsahagatasutta — brahmavihāra + bojjhaṅga integration)

### 4. Anussati (Recollections)
*Six recollections as devotional practice and concentration support.*

No dedicated texts in vault yet.
Texts to add:
- AN 6.10 (Mahānāmasutta — six recollections: Buddha, Dhamma, Saṅgha, sīla, cāga, devatā)
- AN 11.12 (Mahānāmasutta — recollection of the Buddha, expanded)
- AN 6.25 (Anussatiṭṭhānasutta — topics for recollection)
- New mātikā: `six_recollections` (cha anussati)

### 5. Anupubbasikkhā (Gradual Training)
*The step-by-step progression from dāna through sīla to bhāvanā.*

Core texts already in vault: DN 2 (the classic gradual training sutta).
Texts to add:
- MN 27 (Cūḷahatthipadopamasutta — short elephant footprint; gradual training for householders)
- MN 51 (Kandarakasutta — four types of persons in the gradual training)
- AN 8.54 (Dīghajāṇusutta — Byagghapajja; householder's gradual path: dāna → sīla → bhāvanā)
- AN 4.99 (Sikkhāpadasutta — training precepts leading upward)
- New mātikā: `gradual_training` (anupubbasikkhā)

### 6. Maraṇasati (Mindfulness of Death)
*Death contemplation as urgency practice and insight catalyst.*

No dedicated texts in vault yet.
Texts to add:
- AN 6.19 (Maraṇassatisutta — mindfulness of death, first discourse)
- AN 6.20 (Maraṇassatisutta — mindfulness of death, second discourse)
- AN 8.73 (Maraṇassatisutta — death contemplation, eight directions)
- SN 47.13 (Cundasutta — mindfulness at the time of death; Sāriputta's parinibbāna)

### 7. Paritta (Protective Chanting)
*Canonical texts used for protective recitation in the Theravāda tradition.*

Will be partially fulfilled by Snp Chapter 2:
- Snp 2.1 (Ratanasutta — Jewel Discourse)
- Snp 2.4 (Maṅgalasutta — Discourse on Blessings)
- Snp 1.8 (Karaṇīya Mettasutta — already in vault)
Additional:
- AN 4.67 (Ahisutta / Khandhasutta — protection from snakes; protective chanting)
- Create a `paritta/INDEX.md` collecting all protective texts in recitation order

---

## Near-Term: Deepen the Foundation (Phases 6–8)

These are the highest-value additions for a practitioner working in the Theravāda tradition.

### Phase 6 — Remaining Sutta Nipāta Chapters
*The Aṭṭhakavagga is among the oldest strata of the Pali canon; the Metta Sutta (1.8) is already present.*

- Snp Chapter 2 (Cūḷavagga, 14 suttas) — includes Ratana Sutta (paritta), Maṅgala Sutta (blessings)
- Snp Chapter 3 (Mahāvagga, 12 suttas) — Pabbajjā Sutta, Sela Sutta, Dvayatānupassanā (practice-relevant)
- Snp Chapter 4 (Aṭṭhakavagga, 16 suttas) — early poems on non-clinging; highly regarded by scholars and practitioners
- Snp Chapter 5 (Pārāyanavagga, 16 questions) — the Buddha's answers to 16 students; dense and terse
- CSCD: `s0503a.att0.xml` (Niddesa commentary for Atthakavagga/Parāyanavagga)

### Phase 7 — Itivuttaka (112 short discourses)
*Brief, self-contained utterances, each introduced with "This was said by the Blessed One." Ideal for daily reading practice.*

- SuttaCentral IDs: `iti1`–`iti112`; Sujato translation available
- Grouped into 4 nipātas (1–3 factors, 4 factors)
- Cross-links to: four_noble_truths, three_marks, noble_eightfold_path naturally

### Phase 8 — Theragāthā / Therīgāthā (exclamation verses)
*The monks' and nuns' verses of liberation: highly personal, poetic, practice-oriented.*

- Thag: ~264 verses in 21 chapters (Sujato translation)
- Thig: ~73 verses (Sujato translation)
- No formal commentary required; mātikā links to four_sublime_states, nibbāna themes

---

## Medium-Term: Practice-Oriented Expansion (Phase 9)

Migrate suttas specifically chosen to support the seven practice domains above. Each sub-phase creates a thematic reading sequence in `paths/` alongside the canonical texts.

### 9a — Jhāna Deepening
MN 128, AN 9.34–35, MN 8, AN 8.63 + `four_jhanas` mātikā + jhāna reading path

### 9b — Vipassanā Broadening
MN 148, SN 36 (selected suttas), MN 28, MN 140, MN 2 + vipassanā reading path

### 9c — Brahmavihāra Practice
MN 7, DN 13, AN 11.15, AN 4.125–126, SN 46.54 + brahmavihāra reading path

### 9d — Anussati (Recollections)
AN 6.10, AN 11.12, AN 6.25 + `six_recollections` mātikā + anussati reading path

### 9e — Anupubbasikkhā (Gradual Training)
MN 27, MN 51, AN 8.54, AN 4.99 + `gradual_training` mātikā + gradual training path

### 9f — Maraṇasati (Death Contemplation)
AN 6.19–20, AN 8.73, SN 47.13 + maraṇasati reading path

### 9g — Paritta Collection
Cross-reference Snp 2.1, 2.4, 1.8 + AN 4.67 + `paritta/INDEX.md`

---

## Practice Support Tools

These are tools that make the vault actively useful *during* practice, not just for study before or after.

### Thematic Reading Sequences (paths/)
Curated reading paths organized by practice question, created alongside each Phase 9 sub-phase:

- *"Entering jhāna"* → MN 111, AN 9.36, MN 128, DN 9, AN 5.28
- *"Working with the hindrances"* → MN 20, SN 46.51, AN 5.28, DN 2 sections
- *"Ānāpānasati practice arc"* → MN 118, SN 54, AN 10.60, SN 46 (bojjhaṅgas)
- *"Brahmavihāra cultivation"* → Snp 1.8, MN 7, DN 13, AN 11.15, SN 46.54
- *"Understanding the aggregates"* → SN 22 selection, MN 44, MN 43
- *"Path to stream-entry"* → SN 55 selection, DN 2, SN 48.9–10
- *"Contemplating death"* → AN 6.19–20, AN 8.73, SN 47.13

### Personal Practice Notes (Per Sutta)
A `practice/` folder with one note per sutta (or per mātikā), using a minimal template:
- What landed? What was unclear?
- Which passage do you want to memorize or return to?
- Any connection to your sitting practice?
Dataview can then surface notes you've revisited most recently or flagged as important.

### Verse Memorization Log
A note that tracks which verses or passages you're working on committing to memory — sutta reference, the Pali and translation, and a date field for spaced review.

---

## Longer-Term: The Study Environment Matures

These are worth considering only after the near-term content is in place and actually being used.

### Deeper MN Coverage
Remaining candidates beyond those added in Phase 9:
- MN 117 (Mahācattārīsakasutta — right view in full, with mundane/supramundane distinction)
- MN 22 (Alagaddūpamasutta — snake simile, non-clinging to views)

### AN Nipāta Gaps
AN 7, 8, 11 currently have no coverage. Higher priority within them:
- AN 7.65 (seven factors of awakening)
- AN 11.1–5 (Gradual training → Ānāpānasati series)
- AN 8.53 (Dāna, sīla, bhāvanā sequence)

### Vinaya Pātimokkha (Selected Rules)
Not all 227 rules — but the rules with direct bearing on lay or monastic practice context:
- The Bhikkhu Pātimokkha preamble and categories
- Cross-link to five_precepts, eight_precepts
- Thanissaro's translation is freely available

### Abhidhamma (Selective)
- Abhidhammatthasaṅgaha mūla (Bhikkhu Bodhi's "Comprehensive Manual")
- Dhammasaṅgaṇī, Vibhaṅga (mātikā cross-links for five aggregates)

---

## What This Vault Is Not For

Keeping these off the roadmap unless something specific changes:

- **Academic paper ingestion or annotation** — this is a canonical-text reading environment, not a research database
- **NLP or corpus analysis** — the vault itself is not an NLP/corpus-analysis engine; that capability lives in the companion `pali-nlp` repository, which treats the vault as its corpus and writes reading aids (gloss/vocab/SRS, concordance pages) back in. The vault still does not layer AI translation over Sujato's translations, and is still not a public research database.
- **Companion: pali-nlp** — the separate `pali-nlp` repo implements the staged NLP arc (reading aids, pipelines, semantic search, local vector DB). Its work is strictly subordinate to the practice domains, keeping the vault simple, robust, and markdown-native.
- **Sharing or publishing** — the vault is personal; format and density optimized for one reader
- **Automated translation** — Sujato's Suttacentral translations are the authoritative source; don't layer AI translation over them
- **Mobile sync / remote hosting** — complexity cost outweighs benefit for solo use

---

## Evaluation Points

Suggested pauses before commissioning more content:

1. **After Phase 6**: Can you navigate from a mātikā list to a relevant Snp sutta and find the cross-connection useful? If the Snp material doesn't feel integrated, fix that before adding Itivuttaka.
2. **After Phase 8**: Is the three-layer structure still manageable with ~400+ files? Run the link validator; check if INDEX navigation is still quick.
3. **After Phase 9**: Do the thematic reading paths feel natural? Are you using them during practice, or are they just reference material?
4. **Before Vinaya/Abhidhamma**: Has the canonical sutta content reached a point where you're actively using it all? Add Vinaya only if the practice context demands it.
